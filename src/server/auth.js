...
/**
 * Resolves the Bearer token or query-param token to an auth record.
...
/**
 * Elysia plugin: derives `authInfo` on every request.
 * Routes that need auth call `requireAuth(authInfo)`.
 */
...
/**
 * Appends a token-refresh header when the session is close to expiry.
 */
